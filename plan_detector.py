...
def detect_best_plan_and_week(user_profile: Dict, plan_library: List[Dict]) -> Tuple[str, int, float, Dict]:
    """
    Loop through all plans and all weeks, find the best match.
    Returns: plan name, best week index, score, and recommended next 2 weeks
    """
    ...
    upcoming_weeks = []
    if best_plan and "weeks" in best_plan:
        total_weeks = best_plan["weeks"]
        upcoming_weeks = total_weeks[best_week:best_week+2]

    return best_plan_name, best_week, best_score, upcoming_weeks
